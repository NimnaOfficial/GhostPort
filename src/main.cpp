...
#include <windows.h>
#include <pdh.h>
#include <TlHelp32.h> // <-- New: Windows Kernel Thread API
...
// Stasis Tab State
int stasis_mode = 0; // 0 = Docker, 1 = Windows OS
...
// ============================================================================
// [3] HARDCORE SYSTEMS API: WINDOWS THREAD SUSPENSION
// ============================================================================
#ifdef _WIN32
void manipulate_windows_process(DWORD processId, bool suspend)
{
  // 1. Take a snapshot of all running threads in the OS
  ...
  // 2. Loop through the threads
  if (Thread32First(hThreadSnapshot, &threadEntry))
  {
    do
    {
      // 3. If the thread belongs to our target Process ID
      if (threadEntry.th32OwnerProcessID == processId)
      {
        // Open the thread with high privileges
        HANDLE hThread = OpenThread(THREAD_SUSPEND_RESUME, FALSE, threadEntry.th32ThreadID);
        if (hThread)
        {
          // 4. Execute the OS Kernel Freeze/Thaw
          if (suspend)
...
#ifdef _WIN32
  // PowerShell pipeline: Gets top 15 apps (with visual windows), sorted by RAM usage
...
int main(int argc, char *argv[])
{
  ...
  auto scan_btn = Button("Deploy Security Canary", [&]
        ...
        if (!is_scanning) {
            is_scanning = true;
            { std::lock_guard<std::mutex> lock(state_mutex); live_ports.clear(); port_menu_entries.clear(); }
  ...
  auto kill_btn = Button("Terminate Selected Port", [&]
                         {
        if (!port_menu_entries.empty() && selected_port_index >= 0) {
  ...
  auto stop_docker_btn = Button("Graceful Spin Down (SIGTERM)", [&]
                                {
        if (!docker_list.empty() && selected_docker_index >= 0) {
  ...
  auto stasis_subsystem_toggle = Toggle(&stasis_labels, &stasis_mode); // <-- Added the & here!
  ...
  auto freeze_btn = Button("Initiate Cryo-Sleep (Suspend)", [&]
                           {
        if (stasis_mode == 0 && !stasis_docker_list.empty()) {
        ...
        } else if (stasis_mode == 1 && !os_process_list.empty()) {
#ifdef _WIN32
            ...
            manipulate_windows_process(pid, true); // TRUE = Suspend
  ...
  auto thaw_btn = Button("Initiate Thaw Sequence (Resume)", [&]
        ...
        if (stasis_mode == 0 && !stasis_docker_list.empty()) {
            ...
            manipulate_windows_process(pid, false); // FALSE = Resume
            // Small visual cleanup to remove the [FROZEN] tag
  ...
  auto dashboard_renderer = Renderer([&]
        ...
        return vbox({
            ...
            hbox({ window(text(" Engine Status "), vbox({text("Watchdog Daemon : ONLINE") | color(Color::Green), text("IPC Subsystem   : SECURE") | color(Color::Green), text("Docker Engine   : CONNECTED") | color(Color::Green), text("Stasis Chamber  : READY") | color(Color::Green),})) | flex, window(text(" Security Logs "), vbox(std::move(logs))) | flex })
        }); });

  auto network_renderer = Renderer(Container::Vertical({scan_btn, port_menu, kill_btn}), [&]
                                   { return window(text(" Zero-Copy Network Reaper "), vbox({scan_btn->Render() | center, separator(), is_scanning ? (text(" Canary scanning... ") | blink | color(Color::Red) | center) : text(""), port_menu_entries.empty() ? (text(" No malicious ports detected.") | dim | center | flex) : window(text(" Target Lock "), vbox({port_menu->Render(), separator(), kill_btn->Render() | center})) | flex})); });
